...
| [Chess Bot RunPod SDK Component](./chess_bot_runpod_sdk_component.md) | `scripts/runpod_sdk_component.py`, `src/chessbot/runpod_sdk_component.py`, `tests/test_runpod_sdk_component.py` | Separate SDK-based RunPod component with side-by-side capability (gpu/template discovery, provision, status, stop, terminate) maintained modularly from raw API scripts. |
