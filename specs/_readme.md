...
| [Chess Bot Parameter/Config Mappings](./chess_bot_param_config_mappings.md) | `scripts/runpod_cycle_*.sh`, `scripts/runpod_provision.py`, `scripts/runpod_full_train_easy.sh`, `scripts/train_baseline.py` | Canonical map of non-constant runtime controls (env/flags/params), defaults, behaviors, and related commands. |
...
| [Chess Bot Dual Sequence Architecture](./chess_bot_dual_sequence_architecture.md) | `scripts/train_dual_sequence.py`, `src/chessbot/training_dual_sequence.py`, `src/chessbot/model.py`, `src/chessbot/inference.py`, `scripts/infer_move.py` | Dual-model (`white`/`black`) one-shot future-sequence architecture (`H=8` default), winner filtering, draw drop, mate-bias, and side-routed inference compatibility. |
| [Chess Bot Dual Sequence Plan](./chess_bot_dual_sequence_plan.md) | dual-sequence implementation across `scripts/*`, `src/chessbot/*`, `tests/*`, specs updates | Execution-tracked phased implementation/testing plan for dual one-shot sequence training and inference routing. |
...
| [Chess Bot HF Datasets](./chess_bot_hf_datasets.md) | `scripts/hf_dataset_publish.py`, `scripts/hf_dataset_fetch.py`, HF repo `LogicLark-QuantumQuill/chess-bot-datasets` | Canonical HF dataset layout, token auth contract (via Secrets Contract), publish/fetch flow, and stale-version cleanup policy. |
...
| [Chess Bot Cloud Training Script Audit](./chess_bot_cloud_training_script_audit.md) | `scripts/runpod*.sh`, `scripts/vast*.sh`, `deploy/runpod_cloud_training/*.sh`, `deploy/vast_cloud_training/*.sh` | Code-truth audit of cloud training launchers/wrappers with scope, freshness dates, and up-to-date/drift status. |
...
| [Chess Bot RunPod SDK Component](./chess_bot_runpod_sdk_component.md) | `scripts/runpod_sdk_component.py`, `scripts/runpod_sdk_cycle_start.sh`, `scripts/runpod_sdk_cycle_stop.sh`, `scripts/runpod_sdk_cycle_full_smoke.sh`, `src/chessbot/runpod_sdk_component.py`, `tests/test_runpod_sdk_component.py` | Separate SDK-based RunPod component with side-by-side capability (gpu/template discovery, provision/status/stop/terminate, SDK smoke flow) maintained modularly from raw API scripts. |
