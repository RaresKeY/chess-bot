...
- Baseline trainer default objective remains single-step (`--rollout-horizon 1`), not forced 8-ply output.

## Inventory + Status

| Script | Scope | mtime (UTC) | Last Commit | Code Truth | Status |
|---|---|---|---|---|---|
| `scripts/runpod_full_train_easy.sh` | top-level easy full-train wrapper | 2026-02-28 06:48:27 | 2026-02-28T08:53:45+02:00 `d4d52a3` | Sets env defaults and delegates to full HF flow. Defaults are `RUNPOD_FULL_TRAIN_EPOCHS=100` and `RUNPOD_HF_DATASET_SCHEMA_FILTER=auto`. | current with drift |
| `scripts/runpod_cycle_full_train_hf.sh` | primary full RunPod lifecycle + remote training launcher | 2026-03-02 09:48:11 | 2026-03-02T11:48:27+02:00 `bbfbdd5` | Launches `train_baseline_preset.sh` (or direct `train_baseline.py` fallback), supports DDP (`torchrun`), cache-required runtime splice, batch retry ladder, and runtime splice `auto` resolution from cache manifest. | current |
| `deploy/runpod_cloud_training/train_baseline_preset.sh` | in-container training preset | 2026-02-27 10:36:41 | 2026-02-27T12:48:28+02:00 `96a266e` | Trains baseline model, supports HF aggregate inputs, schema-aware path selection, repeated `--train/--val`, runtime splice/cache flags, optional `torchrun`. | current |
...
| `scripts/runpod_cycle_train.sh` | smoke train orchestration lane | 2026-03-01 11:08:56 | 2026-03-01T13:09:51+02:00 `5ad0c09` | Runs remote `train_baseline_preset.sh` and smoke inference check. Optional HF aggregate mode. | current |
...
| `scripts/runpod_full_train_easy_smoke_test.sh` | easy-flow smoke with verify + terminate | 2026-03-02 09:37:15 | 2026-03-02T11:43:34+02:00 `72cbcb6` | Uses full easy flow with smoke-safe overrides (`epochs=1`, row cap, `runtime_max_samples_per_game=auto`), verifies artifacts and termination. | current |
| `scripts/runpod_local_smoke_test.sh` | local Docker smoke harness for RunPod module | 2026-02-28 06:23:05 | 2026-02-28T08:28:27+02:00 `fa1db00` | Launches containerized `train_baseline_preset.sh` with tiny dataset and timing logs. | current |
| `scripts/runpod_cycle_benchmark_matrix.sh` | benchmark matrix/trials lane | 2026-03-01 05:35:32 | 2026-03-01T07:52:54Z `f0eecf1` | Runs multi-trial baseline training with precision/sparsity variants and NCCL fallback logic; launches `train_baseline_preset.sh`. | current |
...
| `deploy/vast_cloud_training/train_baseline_preset.sh` | Vast training preset | 2026-02-27 12:29:11 | 2026-02-27T14:39:00+02:00 `a711fec` | Calls `scripts/train_baseline.py` but passes `--out` (not a valid current CLI flag; current flag is `--output`). | stale or buggy |
...
2. Vast training preset likely fails against current baseline CLI:
   - Uses `--out`.
   - Current trainer expects `--output`.

3. Cloud launchers do not currently enforce the dual-sequence 8-ply regime:
   - No cloud launcher calls `scripts/train_dual_sequence.py`.
   - No launcher sets baseline `--rollout-horizon 8` as default.

## Immediate Recommendations

1. Decide canonical cloud regime explicitly:
   - baseline single-step
   - baseline multistep (`--rollout-horizon 8`)
   - dual-sequence (`train_dual_sequence.py`)

2. Align RunPod defaults and preferred-flow docs:
...
3. Fix Vast preset CLI mismatch:
   - replace `--out` with `--output`.
   - add/refresh a Vast training smoke test lane that exercises the preset script directly.
