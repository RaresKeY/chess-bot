...
FLOW_CLOUD_TYPE="${RUNPOD_CLOUD_TYPE:-SECURE}"
...
FLOW_ROLLOUT_HORIZON="${RUNPOD_FULL_TRAIN_ROLLOUT_HORIZON:-${TRAIN_ROLLOUT_HORIZON:-8}}"
FLOW_CLOSENESS_HORIZON="${RUNPOD_FULL_TRAIN_CLOSENESS_HORIZON:-${TRAIN_CLOSENESS_HORIZON:-${FLOW_ROLLOUT_HORIZON}}}"
...
FLOW_NCCL_SAFE_DEFAULTS="${RUNPOD_FULL_TRAIN_NCCL_SAFE_DEFAULTS:-1}"
FLOW_NCCL_IB_DISABLE="${RUNPOD_FULL_TRAIN_NCCL_IB_DISABLE:-1}"
FLOW_NCCL_P2P_DISABLE="${RUNPOD_FULL_TRAIN_NCCL_P2P_DISABLE:-1}"
FLOW_TORCH_NCCL_ASYNC_ERROR_HANDLING="${RUNPOD_FULL_TRAIN_TORCH_NCCL_ASYNC_ERROR_HANDLING:-1}"
...
runpod_cycle_append_report "${REPORT_MD}" \
  ...
  "- Rollout horizon: \`${FLOW_ROLLOUT_HORIZON}\`" \
  "- Closeness horizon: \`${FLOW_CLOSENESS_HORIZON}\`" \
...
NCCL_SAFE_DEFAULTS="${FLOW_NCCL_SAFE_DEFAULTS:-1}"
if [[ "${NCCL_SAFE_DEFAULTS}" == "1" && "${TRAIN_NPROC_PER_NODE}" -gt 1 ]]; then
  if [[ -z "${NCCL_IB_DISABLE:-}" ]]; then
    export NCCL_IB_DISABLE="${FLOW_NCCL_IB_DISABLE:-1}"
  fi
  if [[ -z "${NCCL_P2P_DISABLE:-}" ]]; then
    export NCCL_P2P_DISABLE="${FLOW_NCCL_P2P_DISABLE:-1}"
  fi
  if [[ -z "${TORCH_NCCL_ASYNC_ERROR_HANDLING:-}" ]]; then
    export TORCH_NCCL_ASYNC_ERROR_HANDLING="${FLOW_TORCH_NCCL_ASYNC_ERROR_HANDLING:-1}"
  fi
  if [[ -z "${TORCH_NCCL_ENABLE_MONITORING:-}" ]]; then
    export TORCH_NCCL_ENABLE_MONITORING="${FLOW_TORCH_NCCL_ENABLE_MONITORING:-1}"
  fi
  if [[ -z "${TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC:-}" ]]; then
    export TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC="${FLOW_TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC:-1800}"
  fi
...
export TRAIN_ROLLOUT_HORIZON="${FLOW_ROLLOUT_HORIZON}"
export TRAIN_CLOSENESS_HORIZON="${FLOW_CLOSENESS_HORIZON}"
...
export TRAIN_MAX_VAL_ROWS="${FLOW_MAX_VAL_ROWS}"
export TRAIN_BEST_CHECKPOINT_OUT="${REMOTE_BEST_CHECKPOINT}"
export TRAIN_EPOCH_CHECKPOINT_DIR="${REMOTE_EPOCH_CHECKPOINT_DIR}"
export TRAIN_EXTRA_ARGS="--epochs ${FLOW_EPOCHS} --early-stopping-patience 0"
export TRAIN_EXTRA_ARGS="${TRAIN_EXTRA_ARGS} --rollout-horizon ${TRAIN_ROLLOUT_HORIZON} --closeness-horizon ${TRAIN_CLOSENESS_HORIZON}"
...
fi
{
  ...
  echo "[runpod-cycle-full-train-hf] rollout_horizon=${TRAIN_ROLLOUT_HORIZON} closeness_horizon=${TRAIN_CLOSENESS_HORIZON}"
...
rc=0
declare -a batch_attempts
batch_attempts=("${TRAIN_BATCH_SIZE}")
if [[ -z "${RUNPOD_FULL_TRAIN_BATCH_SIZE_OVERRIDE:-}" ]]; then
  gpu_name_for_batch="$('/opt/venvs/chessbot/bin/python' - "${REMOTE_CONTEXT_JSON}" <<'PY'
import json, sys
try:
    data = json.load(open(sys.argv[1], "r", encoding="utf-8"))
except Exception:
    print("")
    raise SystemExit(0)
devices = ((data.get("gpu") or {}).get("devices") or [])
if devices and isinstance(devices[0], dict):
    print(str(devices[0].get("name") or ""))
else:
    print("")
PY
)"
  if [[ "${gpu_name_for_batch}" == *"RTX 5090"* ]]; then
    # Prefer a higher effective batch on 5090; fallback on OOM.
    batch_attempts=(4096 3072 2048)
  fi
fi
declare -A _seen_batch=()
declare -a batch_attempts_dedup=()
for _b in "${batch_attempts[@]}"; do
  if [[ "${_b}" =~ ^[0-9]+$ ]] && (( _b > 0 )) && [[ -z "${_seen_batch[${_b}]:-}" ]]; then
    _seen_batch["${_b}"]=1
    batch_attempts_dedup+=("${_b}")
  fi
done
if (( ${#batch_attempts_dedup[@]} == 0 )); then
  batch_attempts_dedup=("${TRAIN_BATCH_SIZE}")
fi
{
  echo "[runpod-cycle-full-train-hf] batch_attempt_plan=${batch_attempts_dedup[*]}"
} >> "${REMOTE_TRAIN_LOG}"

is_oom_failure_tail() {
  tail -n 200 "${REMOTE_TRAIN_LOG}" 2>/dev/null | grep -Eiq \
    'cuda out of memory|out of memory|cublas_status_alloc_failed|cuda error: out of memory'
}
...
if [[ "${preset_has_hf}" == "1" ]]; then
  ...
  for _batch_try in "${batch_attempts_dedup[@]}"; do
    ...
    echo "[runpod-cycle-full-train-hf] batch_attempt_start=${_batch_try} via=preset" >> "${REMOTE_TRAIN_LOG}"
    rc=0
    bash "${TRAIN_PRESET_SCRIPT}" >> "${REMOTE_TRAIN_LOG}" 2>&1 || rc=$?
    if [[ "${rc}" == "0" ]]; then
      break
    fi
    if [[ -n "${RUNPOD_FULL_TRAIN_BATCH_SIZE_OVERRIDE:-}" ]]; then
      break
    fi
    if ! is_oom_failure_tail; then
      break
    fi
    echo "[runpod-cycle-full-train-hf] batch_attempt_oom_retry next_batch=<next>" >> "${REMOTE_TRAIN_LOG}"
  done
else
