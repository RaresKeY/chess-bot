...
FLOW_ROLLOUT_HORIZON="${RUNPOD_BENCH_ROLLOUT_HORIZON:-8}"
FLOW_CLOSENESS_HORIZON="${RUNPOD_BENCH_CLOSENESS_HORIZON:-${FLOW_ROLLOUT_HORIZON}}"
...
FLOW_NCCL_DEBUG="${RUNPOD_BENCH_NCCL_DEBUG:-WARN}"
FLOW_TORCH_NCCL_ASYNC_ERROR_HANDLING="${RUNPOD_BENCH_TORCH_NCCL_ASYNC_ERROR_HANDLING:-1}"
FLOW_TORCH_NCCL_ENABLE_MONITORING="${RUNPOD_BENCH_TORCH_NCCL_ENABLE_MONITORING:-1}"
FLOW_TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC="${RUNPOD_BENCH_TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC:-60}"
FLOW_TORCH_NCCL_DUMP_ON_TIMEOUT="${RUNPOD_BENCH_TORCH_NCCL_DUMP_ON_TIMEOUT:-1}"
FLOW_TORCH_NCCL_TRACE_BUFFER_SIZE="${RUNPOD_BENCH_TORCH_NCCL_TRACE_BUFFER_SIZE:-2000}"
FLOW_EXPECTED_GIT_SHA="$(git -C "${REPO_ROOT}" rev-parse HEAD)"

if ! [[ "${FLOW_ROLLOUT_HORIZON}" =~ ^[0-9]+$ ]] || (( FLOW_ROLLOUT_HORIZON < 1 )); then
  echo "[runpod-bench-matrix] invalid RUNPOD_BENCH_ROLLOUT_HORIZON='${FLOW_ROLLOUT_HORIZON}' (expected integer >=1)" >&2
  exit 2
fi
if ! [[ "${FLOW_CLOSENESS_HORIZON}" =~ ^[0-9]+$ ]] || (( FLOW_CLOSENESS_HORIZON < 1 )); then
  echo "[runpod-bench-matrix] invalid RUNPOD_BENCH_CLOSENESS_HORIZON='${FLOW_CLOSENESS_HORIZON}' (expected integer >=1)" >&2
  exit 2
fi
...
- rollout_horizon: \`${FLOW_ROLLOUT_HORIZON}\`
- closeness_horizon: \`${FLOW_CLOSENESS_HORIZON}\`
...
- nccl_debug: \`${FLOW_NCCL_DEBUG}\`
- torch_nccl_async_error_handling: \`${FLOW_TORCH_NCCL_ASYNC_ERROR_HANDLING}\`
- torch_nccl_enable_monitoring: \`${FLOW_TORCH_NCCL_ENABLE_MONITORING}\`
- torch_nccl_heartbeat_timeout_sec: \`${FLOW_TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC}\`
- torch_nccl_dump_on_timeout: \`${FLOW_TORCH_NCCL_DUMP_ON_TIMEOUT}\`
- torch_nccl_trace_buffer_size: \`${FLOW_TORCH_NCCL_TRACE_BUFFER_SIZE}\`
...
for raw_trial in "${FLOW_TRIALS[@]}"; do
  ...
  trial_exit=0
  trial_batch_used=""
  train_extra_args="--epochs ${FLOW_EPOCHS} --early-stopping-patience 0 --rollout-horizon ${FLOW_ROLLOUT_HORIZON} --closeness-horizon ${FLOW_CLOSENESS_HORIZON} --progress --verbose"
  ...
  if [[ "${trial_status}" == "ok" && "${FLOW_ROLLOUT_HORIZON}" != "1" ]]; then
    case "${trial_base}" in
      fp32_sparse|fp16_sparse|bf16_sparse|sparsity|fp16_2to4|bf16_2to4)
        trial_status="skipped"
        trial_exit=0
        ;;
    esac
...
if [[ "${FLOW_DISTRIBUTED_BACKEND}" == "nccl" ]]; then
  export NCCL_DEBUG="${FLOW_NCCL_DEBUG}"
  export TORCH_NCCL_ASYNC_ERROR_HANDLING="${FLOW_TORCH_NCCL_ASYNC_ERROR_HANDLING}"
  export TORCH_NCCL_ENABLE_MONITORING="${FLOW_TORCH_NCCL_ENABLE_MONITORING}"
  export TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC="${FLOW_TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC}"
  export TORCH_NCCL_DUMP_ON_TIMEOUT="${FLOW_TORCH_NCCL_DUMP_ON_TIMEOUT}"
  export TORCH_NCCL_TRACE_BUFFER_SIZE="${FLOW_TORCH_NCCL_TRACE_BUFFER_SIZE}"
fi
