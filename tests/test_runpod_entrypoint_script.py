...
class RunpodEntrypointScriptTests(unittest.TestCase):
    def test_entrypoint_handles_non_git_repo_dir_and_skips_inference_without_torch(self):
        ...
        self.assertIn("Repo dir is not writable by ${RUNNER_USER}; attempting ownership/permission repair", text)
        self.assertIn("Repo dir remains non-writable for ${RUNNER_USER}", text)
